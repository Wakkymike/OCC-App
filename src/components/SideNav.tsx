...
export default function SideNav() {
  ...
  return (
    <aside className="h-screen sticky top-0 shrink-0 border-r border-border bg-background flex flex-col w-16 md:w-56 transition-[width] duration-200">
      {/* Header */}
      <div className="px-3 md:px-4 pt-4 pb-3 border-b">
        <h2 className="hidden md:block text-xs font-bold uppercase tracking-widest text-muted-foreground">
          Navigation
        </h2>
      </div>

      {/* Nav items */}
      <nav className="flex-1 overflow-y-auto py-2 px-1.5 md:px-2">
        {visibleItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`shrink-0 ${isActive ? '' : 'opacity-70'}`}>{item.icon}</span>
      ...
      <div className="border-t p-2 md:p-3 space-y-3">
        {/* Theme selector */}
